...
The query string and fragment are never collected, so tokens and email addresses in a URL never leave the browser. Neither field is rule-usable; use the `path` field (supplied by your own edge at verdict time) to write path rules.
